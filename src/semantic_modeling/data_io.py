    ...
    if dataset not in _data_io_vars['data_tables']:
        ...
        if cache_file.exists():
            ...
            for raw_tbl in raw_tables:
                tbl, sm = R2RML.load_from_file(mapping_dir / f"{raw_tbl.id}-model.yml").apply_build(raw_tbl)
                tables.append(tbl)
...
if __name__ == '__main__':
    ...
    (data_dir / "models-viz" / "pdf").mkdir(exist_ok=True, parents=True)
    (data_dir / "tables-viz").mkdir(exist_ok=True, parents=True)

    for sm in get_semantic_models(dataset):
        sm.graph.render2pdf(data_dir / f"models-viz/pdf/{sm.id}.pdf")
        sm.graph.render2img(data_dir / f"models-viz/{sm.id}.png")

    for tbl in get_data_tables(dataset):
        if not tbl.id.startswith("s10"):
            continue
