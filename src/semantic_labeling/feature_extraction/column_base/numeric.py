...
from scipy.stats import ks_2samp, mannwhitneyu, ttest_ind, pearsonr, linregress
...
def average_dist_test(col1: Column, col2: Column) -> float:
    if len(col1.get_numeric_data()) > 1 and len(col2.get_numeric_data()) > 1:
        mean_1 = np.mean(col1.get_numeric_data())
        mean_2 = np.mean(col2.get_numeric_data())
        return (mean_1 - mean_2) / max(mean_1, mean_2)
        # return linregress(col1.get_numeric_data(), col2.get_numeric_data())[2]
    return 0.0


def linregress_test(col1: Column, col2: Column) -> float:
    if len(col1.get_numeric_data()) > 1 and len(col2.get_numeric_data()) > 1:
        print(len(col1.get_numeric_data()), len(col2.get_numeric_data()))
        return linregress(col1.get_numeric_data(), col2.get_numeric_data())[2]
    return 0.0
