...
class SemanticTypeDB(object):
    ...
    SIMILARITY_METRICS = [ "average_dist_test", "num_ks_test", "num_mann_whitney_u_test", "num_jaccard", "text_jaccard",
                          "text_tf-idf"
                          ]
    instance = None
    ...
    def _compute_feature_vectors(self, col: Column, refcols: List[Column], refcol_stypes: List[str]):
        ...
        for i, refcol in enumerate(refcols):
            features.append([
                ...
                # numeric features
                numeric.average_dist_test(refcol, col),
...
class TfidfDatabase(object):
    ...
    def create(tokenizer, columns: List[Column]) -> 'TfidfDatabase':
        ...
        for tf_col in tf_cols:
            for w in tf_col:
                invert_token_idx[w.lower()] += 1
                token_count[w.lower()] += 1
